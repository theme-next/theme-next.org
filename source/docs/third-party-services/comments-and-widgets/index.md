...
NexT provide some perfect comment system, you can use them through set `comments_service` to a comment service name that you want to integrate. Such as:
```yml
# Comments Service: Disqus Changyan Valine LiveRe Gitment Gitalk facebook_comments_plugin vkontakte disqusjs.
# You can set one value to enable.
# Example: comments_service: Disqus.
comments_service: Disqus
```
And then, add those special configurations that you can find below to the `_config.yml`.
...
4. Add the obtained Disqus shortname (`shortname`), and edit other configurations in `disqus` section in the {% label primary@theme config file %} as following:
...
4. Add the obtained Disqus API Key (`apikey`) and Shortname (`shortname`), and edit other configurations in `disqusjs` section in the {% label primary@theme config file %} as following:
    ```yml next/_config.yml
    # DisqusJS
...
3. Add Client ID (`client_id`) and Client secret (`client_secret`) in step 1, add your Github username (`github_id` and `admin_user`) and the created repository name (`repo`) in step 2, and edit other configurations in `gitalk` section in the {% label primary@theme config file %} as following:
...
3. Add the obtained APP ID (`appid`) and APP Key (`appkey`), and edit other configurations in `valine` section in the {% label primary@theme config file %} as following:
...
3. Add Client ID (`client_id`) and Client secret (`client_secret`) in step 1, add your Github username (`github_user`) and the created repository name (`github_repo`) in step 2, and edit other configurations in `gitment` section in the {% label primary@theme config file %} as following:
...
2. And add the obtained APP ID (`appid`) and APP KEY (`appkey`) to the `changyan` section in the {% label primary@theme config file %} as following:
...
As you can see, all of our comment system code is in `layout/_third-party/comments/{service-name}` folder.
- index.swig: the scripts needed by the comment system.
- comment.swig: this part of the code will be in the right place to display comment content.
- count.swig: display the number of comments in the title.
So you can easily add any comment system through create those files in `layout/_third-party/comments/{your-comment-systen}`. And set `comments_service` to `{your-comment-systen}`.
Of course, we look forward to your PR.
